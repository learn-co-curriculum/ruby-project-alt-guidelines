class PantryApp
    ...
    def home_page
        ...
        prompt.select("What would you like to do?") do |menu|
            menu.choice "View Pantries", -> {find_helper}
            menu.choice "Manage Your Favorite Pantries (Add, Delete)", -> {favorites_helper}
    ...
    def find_helper
        ...
        puts "Provision".black.on_green
        prompt.select("What would you like to do?") do |menu|
            menu.choice "View Pantries by Borough", -> {borough_helper}
            menu.choice "View Pantry Descriptions", -> {description_helper}
            menu.choice "View Pantries with Fresh Produce", -> {fresh_helper}
    ...
    def fresh_helper
        system 'clear'
        puts "Provision".black.on_green
        fresh_data = Resource.fresh_produce
        fresh_pantries_names = fresh_data.map{|borough| borough.name}
        user_fresh_pantry = prompt.select("These pantries offer fresh produce. Select a pantry for more information:", fresh_pantries_names, per_page: 20)
        pantry = fresh_data.find{|fresh_pantry| fresh_pantry.name == user_fresh_pantry}
